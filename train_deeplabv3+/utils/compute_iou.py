...
def compute_mIoU(gt_dir, pred_dir, set, devkit_dir='', source_domain='synthia', print_every=False):
    """
    ...
    for ind in range(len(gt_imgs)):
        ...
        if len(label.flatten()) != len(pred.flatten()):
            ...
            continue
        hist_tmp = fast_hist(label.flatten(), pred.flatten(), num_classes)
        hist += hist_tmp
        # [user] print each image miou 
        if print_every:
            print(('{:s} {:0.2f}'.format(gt_imgs[ind].split('/')[-1], 100*np.nanmean(per_class_iu(hist_tmp)))))
        
        if ind > 0 and ind % 10 == 0 and not print_every:
