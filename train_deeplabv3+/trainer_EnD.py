...
class AD_Trainer(nn.Module):
    def __init__(self, args):
        ...
        self.G = Deeplabv3(num_classes=args.num_classes, output_stride=16, backbone=args.backbone)
    ...
    def gen_update(self, images, labels, i_iter, T=1.0): # T is the temperature
        # criterion = nn.KLDivLoss(size_average=False)
        criterion = nn.KLDivLoss()
        self.gen_opt.zero_grad()
        pred = self.G(images)
        # original
        # pred = self.sm(self.interp(pred)) + 1e-6
        # for temperature 
        pred = self.sm(self.interp(pred)) + 1e-6

        labels = labels.type(torch.float32)/100.0 # labels: [B:10, C:19, 256, 512]
        

        labels = labels.permute(1, 0, 2, 3)
        pred = pred.permute(1, 0, 2, 3)
        # print('labels_value: ', labels[:, 0, 0, 0])
        # print('pred_value: ', pred[:, 0, 0, 0])
        loss = F.kl_div(pred.log(), labels, reduction='mean') 
        # from EnD
        """
