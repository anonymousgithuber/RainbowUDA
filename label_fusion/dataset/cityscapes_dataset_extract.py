...
class cityscapesExtraction(data.Dataset):
    def __init__(self, root, list_path, set, max_iters=None, ignore_label=255, source_domain='gta5'):
        self.root = root
        ...
        for name in self.img_ids:
            ...
            # (user) file structure
            label_file =osp.join(self.root, "pseudo_label/%s/%s/%s" % (self.source_domain.upper(), self.set, name))
    ...
    def __getitem__(self, index):
        ...
        # (user) change to npz label_array (1024 * 2048 * 19)
        try:
            # normalize certainty map
            label_array = np.load(datafiles["label"]+'.npz')["arr_0"]
            label_array = np.asarray(label_array, np.float32)
            label_array_sum = np.sum(label_array, axis=2)
            label_array_sum = np.repeat(label_array_sum[:, :, np.newaxis], 19, axis = 2)
            label_array = (label_array * 100)/label_array_sum
        except:
            # print('no label_array')
            label_array = np.zeros([1024, 2048, 19])
